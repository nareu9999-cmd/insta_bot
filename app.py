...
st.set_page_config(page_title="AI 인플루언서 추출기 엔터프라이즈", layout="wide")
st.title("✨ AI 인플루언서 추출기 (실무 정교화 버전)")
st.markdown("데이터 파싱 예외 처리 및 AI 한도 방어 로직이 강화된 프로페셔널 버전입니다.")
...
with tab1:
    ...
    if run_btn:
        if not search_input:
            ...
            # --- 1단계: 모드별 후보군 1차 수집 ---
            if search_mode == "해시태그 검색 (# 기반)":
                with st.spinner("🧠 해시태그 및 연관 트렌드 피드에서 후보군을 수집 중입니다..."):
                    try:
                ...
                with st.spinner(f"🧠 통합 검색창에서 '{search_input}' 관련 유저 계정들을 수집 중입니다..."):
                    try:
                        ...
                        st.error(f"키워드 검색 단계 오류: {e}")
                        st.stop()

            # --- 2단계: 프로필 정밀 2차 검증 (하한선 실시간 필터링) ---
            with st.spinner(f"🕵️ 일반인 계정 차단 중... {len(raw_usernames)}명의 상세 프로필 실시간 유효성 검증 중..."):
                verified_user_map = {}
                try:
                    candidate_list = list(raw_usernames)[:target_count * 4]
                    if not candidate_list:
                        st.warning("⚠️ 1차 탐색된 계정 후보가 없습니다. 검색 조건을 변경해 주세요.")
                        st.stop()
                    ...
                    for p_item in apify_client.dataset(profile_dataset_id).iterate_items():
                        ...
                        # 🎯 실무용 필수 필터: 설정한 최소 팔로워 수 미만 계정 원천 누락
                        if not (min_follower <= followers <= max_follower):
                            continue
                            
                        # 맞팔 위주의 스팸/비활동성 일반 계정 차단
                    ...
                    st.error(f"2차 프로필 검증 단계 오류: {e}")
                    st.stop()

            # --- 3단계: AI 매칭도 검증 및 리스트 구성 ---
            with st.spinner("🤖 캠페인 적합도 최종 분석 및 데이터 바인딩 중..."):
                final_list = []
                if not verified_user_map:
                    st.warning(f"⚠️ 설정하신 최소 팔로워 {min_follower:,}명 기준을 충족하는 인플루언서가 후보군에 없습니다. 범위를 조정해 주세요.")
                else:
                    ...
                    for username, info in verified_user_map.items():
                        ...
                        # 무료 API 429 제한 완화를 위한 정밀 딜레이 세팅
                        time.sleep(4.3)
                        ...
                        반드시 아래 세 줄 양식만 출력해. 다른 꼬리말이나 설명은 절대 하지 마.
                        점수: [0~100 숫자]
                        추천: [YES 또는 NO]
                        ...
                        try:
                            ...
                            # 💡 정교화 패치: AI 에러 발생 시 프로그램 멈춤 방지
                            if "429" in str(ai_err):
                                bio_summary = "⏳ 한도 초과 (종량제 전환 권장)"
                            else:
                                bio_summary = f"확인 실패 ({str(ai_err)[:15]})"
                    ...
                    if final_list:
                        ...
                        status_text.empty()
                        st.success(f"🎉 검증 완료! 하한선({min_follower:,}명) 이상의 유효 인플루언서 명단입니다.")
                        ...
                        st.download_button("📥 정밀 검증 명단 다운로드(CSV)", data=csv, file_name=f"인플루언서추출_{search_input}.csv", mime="text/csv")
                    else:
                        st.error("❌ 조건에 맞는 계정이 발견되지 않았습니다. 필터 범위를 완화해 주세요.")
